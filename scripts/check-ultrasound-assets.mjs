...
const expectedAssets = [
  { file: 'proximal.webp', bytes: 10316, sha256: '5cca43dce93fc4fa160a7e75de5cf55c8e26988ee9e9a0e8f273d41a85e1c20b' },
  { file: 'proximal-mid.webp', bytes: 9616, sha256: '168f341a8705a991ab7f60ac9d924a520670d9a5d3cb4e536e43c121d81403ef' },
  { file: 'mid.webp', bytes: 10296, sha256: 'd75c7b6da5196f1b00ae7fc133dd45f42a67d022c22b98ec578e6926b2385447' },
  { file: 'mid-distal.webp', bytes: 10386, sha256: '427b346f1fd34a857005d13126315813e68109185ec576a015245276763c8f0b' },
  { file: 'distal.webp', bytes: 10014, sha256: '9a4bb737bcdbb02367488eeea66301dac71c776efb7cc629fe24cab9178037c1' },
];
...
function verifyStack(baseDir, label) {
  ...
  for (const expected of expectedAssets) {
    ...
    const actualSha256 = inspectWebP(path, expected, `${label} ${expected.file}`);
    console.log(`ASSET DIGEST ${label} ${expected.file} ${expected.bytes} ${actualSha256}`);
    if (actualSha256 !== expected.sha256) mismatches.push(`${expected.file}: ${actualSha256}`);
    totalBytes += expected.bytes;
  }
  assert(totalBytes < 150_000, `${label} stack exceeds the P1 total asset budget`);
  assert(mismatches.length === 0, `${label} SHA-256 mismatch(es): ${mismatches.join('; ')}`);
}
